...
namespace Ricky8955555.CoolQ
{
    ...
    class SplitMessage
    {
        ...
        public static SplitMessage Parse(IContentfulMessage message)
        {
            var msg = message.Parse(); // 将 Message 转换成 ComplexMessage
            var currentUser = PluginContext.Current.Bot.CurrentUser; // 定义 currentUser 为 当前用户
            ...
            if (msg.Contains(currentUser.At())) // 判断是否存在 At，且对象为机器人
            {
                ...
                messageAt.Target.Equals(currentUser)) // 确定 At 的目标为当前用户
                {
